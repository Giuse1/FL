...
from FL.torch_dataset import ValidationDataset, getValidationDataloader
...
def train_model(global_model, criterion, num_rounds=50, local_epochs=1):
    ...
    valloader_list = getValidationDataloader(path='data_test/', transform=transform,batch_size=8, shuffle=True )
    ...
    for round in range(num_rounds):
        ...
        for phase in ['train', 'val']:
            if phase == 'train':
                ...
                val_loss_r, val_accuracy_r = model_evaluation(model=global_model,dataloader_list=valloader_list, criterion=criterion)
